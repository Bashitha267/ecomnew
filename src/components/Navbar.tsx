...
export const Navbar: React.FC<NavbarProps> = ({ onNavClick }) => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      // Transition from transparent to white background when scrolling past 80px
      ...
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          scrolled
            ...
            : "bg-transparent text-white border-b border-white/15 py-5 bg-gradient-to-b from-black/50 via-black/25 to-transparent"
        }`}
      >
        <div className="max-w-[1700px] mx-auto px-4 md:px-8 flex items-center justify-between">
          ...
          <div className="flex items-center md:hidden">
            <button
              ...
              className={`p-1 hover:opacity-70 transition-opacity ${scrolled ? "text-black" : "text-white"}`}
          ...
          <div className="text-center flex-1 md:flex-initial">
            ...
            >
              <h1
                className={`text-2xl md:text-3xl lg:text-4xl font-extrabold tracking-[0.22em] uppercase font-display leading-none transition-all duration-300 ${
          ...
          {/* Right Utilities: Currency Selector, Account, Search, Cart */}
          <div className="flex items-center space-x-4 md:space-x-6 text-[13px]">
        ...
        {isMobileMenuOpen && (
          <div className={`md:hidden border-t px-6 py-6 animate-fadeIn ${
            scrolled ? "bg-white text-black border-neutral-100" : "bg-black/90 text-white border-white/10 backdrop-blur-md"
          }`}>
            <nav className="flex flex-col space-y-4 text-center font-serif text-sm tracking-[0.2em]">
              {navLinks.map((link) => (
                <button
                  key={link.id}
                  onClick={() => handleLinkClick(link.id)}
                  className="py-2 hover:opacity-60 transition-opacity border-b border-neutral-50/20 uppercase"
                >
                  {link.label}
                </button>
              ))}
